...
int main(int argc, const char* argv[])
{
	...
	struct timeval t_start, t_end;
 	float mTime =0;
 	float avg, sample, last_sample, last_output, filter_out;
...
//	const int clk_div = 34 ;
	...
	i = 0;
		...
		for(j = 0 ; j < SAMPLE_SIZE ; j++) {
			s = buffer_AIN_1[j];
			sample = (OFFSET - ((float)s / 4095.0f) * 1.8f);
			//avg += fabs(OFFSET - ((float)sample / 4095.0f) * 1.8f);
			filter_out = highPassFilter(last_output, sample, last_sample);
			//fprintf(adc_file, "%0.3f, %0.3f\n", filter_out, sample);
			avg += fabs(filter_out);
			last_sample = sample;
			last_output = filter_out;
